  ...
  def feed_message(activity)
    ...
    when "State"
      ...
      %(
        #{view_state}: 
        #{h(state.body)}
       #{content_tag("div",time_ago_in_words(state.created_at) ,"class"=>"time")}
      )
    when "Post"
      post = activity.item
      %(
       ...
       #{content_tag("div",time_ago_in_words(post.created_at),"class"=>"time")}
      )
    when "Comment"
      ...
      %(
       ...
       #{content_tag("div",time_ago_in_words(comment.created_at, include_seconds = false),"class"=>"time")}
      )
    end
  end
